...
void main(void) {
	...
	printf("\nРезультат вычислений: %lld\n", arithmeticResult);
	clock_t end = clock();

	double executionTime = ((double)(end - start)) / CLOCKS_PER_SEC;
	printf("На выполнение вычислений потребовалось %g секунд\n", executionTime);
}
...
long long parseAndCalculateExpression(char* expression) {
	...
	// Пока в стеке есть операторы, производим с ними действия
	...
	// Результат всех вычислений - последнее оставшееся в стеке число (если изначальное выражение корректно)
